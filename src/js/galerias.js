...
async function iniciarUsuarios(){
    ...
    try{
        const url = ruta + '/admin/api/usuariosNoFoto';
...
async function buscarUsuarios(cadena){
    ...
    console.log(datos);
    try{
        const url = 'http://localhost:3000/admin/api/buscaUsuariosNoFoto';
        const resultado = await fetch(url, {
            method: 'POST',
            body:datos
        });
...
function mostrarUsuarios(usuarios){
    ...
    borrarTabla();
        usuarios.forEach( usuario =>{
            ...
            botonCrear.classList.add("boton");
            idUsuario = id;
            botonCrear.textContent = "Nueva Galeria";
                ...
                botonCrear.addEventListener('click', ()=>{
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            $.post("http://localhost:3000/admin/galerias/crear",
                                {
                                    id: idUsuario
                                },
...
async function crearGaleria(id){
    ...
    try{
        console.log(datos);
        const url = `http://localhost:3000/admin/galerias/nueva`;
        const resultado = await fetch(url, {
            method: 'POST',
            body:datos
        });
